                ...
                <ul className='flex animate-carousel gap-4'>
                    {
                        products.map((item) => (
                            <li key={item.id} className='relative aspect-square h-[30vh] max-h-[275px] w-2/3 max-w-[475px] flex-none md:w-1/3'>
                                <Link className='relative h-full w-full' href={`/product/${item.id}`}>
                                    <div className='group flex h-full w-full items-center justify-center overflow-hidden rounded-lg border bg-white hover:border-blue-600 dark:bg-black relative border-neutral-200 dark:border-neutral-800'>
                                    <Image className='relative bg-[#ffffff] h-full w-full object-contain transition duration-300 ease-in-out group-hover:scale-105' fill src={item.image} alt='' sizes={`(min-width: 768px) 33vw, 50vw`} style={{position: 'absolute', height: '100%', width: '100%', left: 0, top: 0, right: 0, bottom: 0, color: "transparent"}}/>
